...
export function ApprovalGate({
  ...
  const locked = run.status === "approved" || run.status === "rejected";
  ...
  return (
    <section className="rounded-2xl border border-copper/40 bg-white/80 p-5">
      <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-copper">Human approval</p>
      ...
      <p className="mt-2 text-sm text-ink-soft">{run.plan.approvalReason}</p>
      {locked ? (
